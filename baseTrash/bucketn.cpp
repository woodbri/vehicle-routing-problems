...
/***************************** DUMP PRINTS PLOTS   ********/
   ...
   Twpath<Trashnode>& BucketN::Path() { return path; }
   const Twpath<Trashnode>& BucketN::Path() const  { return path; }
   const Twpath<Trashnode>& BucketN::getpath() const  { return path; }
...
/* functions to find position */
   ...
   bool BucketN::in(const Trashnode &node) const { return in(node.getnid()); }
//   bool BucketN::in(const Order &o) const { return in( o.getdid());}
   int BucketN::pos(const Trashnode &node) const { return pos(node.getnid()); }
/*
...
/*************** path operations aka to use Vehicke as a bucket ******************/
    ...
    void BucketN::push_back(Trashnode pathstop) { path.push_back(pathstop); }
    void BucketN::insert(const Trashnode &pathstop,int at) { std::cout<<"INSERTING"<<at<<"\n";path.insert(pathstop,at); }

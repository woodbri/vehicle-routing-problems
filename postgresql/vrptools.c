...
static int fetch_vehicle_columns(SPITupleTable *tuptable,
    ...
    vehicle_columns->dumpservicetime = SPI_fnumber(SPI_tuptable->tupdesc, "dumpservicetime");
...
static int solve_trash_collection(
    ...
    if (SPIplan  == NULL) {
        elog(ERROR, "solve_trash_collection: couldn't create query plan for cpntainers via SPI");
        return -1;
    }

    if ((SPIportal = SPI_cursor_open(NULL, SPIplan, NULL, NULL, true)) == NULL) {
        elog(ERROR, "solve_trash_collection: SPI_cursor_open('%s') returns NULL", container_sql);
        return -1;
    }
...
/***********************************************************************/
    ...
    if (SPIplan  == NULL) {
        elog(ERROR, "solve_trash_collection: couldn't create query plan for otherlocs via SPI");
        return -1;
    }

    if ((SPIportal = SPI_cursor_open(NULL, SPIplan, NULL, NULL, true)) == NULL) {
        elog(ERROR, "solve_trash_collection: SPI_cursor_open('%s') returns NULL", otherloc_sql);
        return -1;
    }
...
/***********************************************************************/
    ...
    if (SPIplan  == NULL) {
        elog(ERROR, "solve_trash_collection: couldn't create query plan for vehicles via SPI");
        return -1;
    }

    if ((SPIportal = SPI_cursor_open(NULL, SPIplan, NULL, NULL, true)) == NULL) {
        elog(ERROR, "solve_trash_collection: SPI_cursor_open('%s') returns NULL", vehicle_sql);
        return -1;
    }
...
/***********************************************************************/
    ...
    if (SPIplan  == NULL) {
        elog(ERROR, "solve_trash_collection: couldn't create query plan for ttimes via SPI");
        return -1;
    }

    if ((SPIportal = SPI_cursor_open(NULL, SPIplan, NULL, NULL, true)) == NULL) {
        elog(ERROR, "solve_trash_collection: SPI_cursor_open('%s') returns NULL", ttime_sql);
        return -1;
    }

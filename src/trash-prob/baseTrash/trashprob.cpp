...
void TrashProb::addContainers( container_t *_containers, int count ) {
    ...
    for ( int i = 0; i < count; ++i ) {
	   ...
	   errorStr= "Container #"+numbertoString(c.id)+": expecting Lat/Lon coordinates, other was given";
	   errorsFound.push_back(errorStr);
	   dataHasError=true;
        };

        if (not dataIsLatLon and  isLatLon(c.x,c.y) ) {
	   errorStr="Container #"+numbertoString(c.id)+": expecting NOT lat/long coordinates, Lat/Lon was given";
	   errorsFound.push_back(errorStr);
	   dataHasError=true;
        };

   	if (c.open > c.close) {
	   errorStr = "Container #"+numbertoString(c.id)+": is Closing before it Opens";
	   errorsFound.push_back(errorStr);
	   dataHasError=true;
        };

   	if (c.demand == 0) {
	   errorStr = "Container #"+numbertoString(c.id)+": Is empty, Demand should be non zero positive";
	   errorsFound.push_back(errorStr);
	   dataHasError=true;
        };

   	if (c.demand < 0) {
	   errorStr = "Container #"+numbertoString(c.id)+": Demand <0, should have a non zero positive value";
	   errorsFound.push_back(errorStr);
	   dataHasError = true;
        };
...
void TrashProb::addOtherlocs( otherloc_t *_otherlocs, int count ) {
    ...
    for ( int i = 0; i < count; ++i ) {
       ...
       if (dataIsLatLon and not isLatLon(c.x,c.y) ) {
           errorStr= "Other Location #"+numbertoString(c.id)+": expecting Lat/Lon coordinates, other was given";
           errorsFound.push_back(errorStr);
           dataHasError=true;
       };

       if (not dataIsLatLon and  isLatLon(c.x,c.y) ) {
           errorStr="Other Location #"+numbertoString(c.id)+": expecting NOT lat/long coordinates, Lat/Lon was given";
           errorsFound.push_back(errorStr);
           dataHasError=true;
       };

       if (c.open > c.close) {
           errorStr = "Other Location #"+numbertoString(c.id)+": is Closing before it Opens";
           errorsFound.push_back(errorStr);
           dataHasError=true;
       };
...
void TrashProb::addVehicles( vehicle_t *_vehicles, int count ) {
    ...
    for ( int i = 0; i < count; ++i ) {
       ...
       if (not ( v.start_id >= 0 and v.dump_id >= 0 and v.end_id >= 0 and
           ...
           if (v.start_id < 0) {
              errorStr= "Truck #"+numbertoString(v.vid)+": has negative starting place id ";
              errorsFound.push_back(errorStr);
           } else {
		...
		if (otherlocs.hasId(v.dump_id) ) {
           ...
           if (v.end_id < 0) {
              errorStr= "Truck #"+numbertoString(v.vid)+": has negative ending place id ";
              errorsFound.push_back(errorStr);
           } else {
		if (otherlocs.hasId(v.end_id) ) {
           ...
           if (v.starttime > v.endtime) {
              errorStr= "Truck #"+numbertoString(v.vid)+": shift ends before it starts (start time > end time)";
              errorsFound.push_back(errorStr);
           };

           if (v.capacity <= 0) {
              errorStr= "Truck #"+numbertoString(v.vid)+": has no capacity or has negative capacity)";
              errorsFound.push_back(errorStr);
           };

           
           if (v.dumpservicetime <= 0) {
              errorStr= "Truck #"+numbertoString(v.vid)+": has negative service time at the dump)";
              errorsFound.push_back(errorStr);
           };
